...
import MDAnalysis as mda # Import MDAnalysis for trajectory loading
...
def parse_arguments():
    ...
    frame_group = parser.add_argument_group('Frame Selection Options')
    frame_group.add_argument("--start_frame", type=int, default=None, help="Starting frame index for analysis (0-based).")
    frame_group.add_argument("--end_frame", type=int, default=None, help="Ending frame index for analysis (exclusive). If not specified, goes to the end.")
...
def _run_analysis_workflow(args):
    ...
    try:
        ...
        # --- Load Universe for Trajectory Analysis ---
        logger.info(f"Loading MDAnalysis Universe from {psf_file} and {dcd_file}...")
        try:
            universe = mda.Universe(psf_file, dcd_file)
            n_frames_total = len(universe.trajectory)
            
            if n_frames_total < 2:
                error_msg = f"Trajectory contains too few frames ({n_frames_total}). At least 2 frames are required."
                logger.error(error_msg)
                raise ValueError(error_msg)
                
            logger.info(f"Successfully loaded trajectory with {n_frames_total} frames.")
            
            # Determine frame range for analysis
            start_frame_arg = args.start_frame
            end_frame_arg = args.end_frame
            
            actual_start = start_frame_arg if start_frame_arg is not None else 0
            actual_end = end_frame_arg if end_frame_arg is not None else n_frames_total
            
            # Validate range
            if actual_start < 0:
                error_msg = f"Invalid start_frame: {actual_start}. Must be >= 0."
                logger.error(error_msg)
                raise ValueError(error_msg)
                
            if actual_end > n_frames_total:
                logger.warning(f"Specified end_frame ({actual_end}) exceeds total frames ({n_frames_total}). Using {n_frames_total} instead.")
                actual_end = n_frames_total
                
            if actual_start >= actual_end:
                error_msg = f"Invalid frame range: start_frame ({actual_start}) must be less than end_frame ({actual_end})."
                logger.error(error_msg)
                raise ValueError(error_msg)
                
            # Store frame range in database
            set_simulation_metadata(db_conn, "start_frame", str(actual_start))
            set_simulation_metadata(db_conn, "end_frame", str(actual_end))
            set_simulation_metadata(db_conn, "total_frames", str(n_frames_total))
            
            logger.info(f"Analysis will use frame range: {actual_start} to {actual_end} (total: {actual_end - actual_start} frames)")
            
        except Exception as e:
            error_msg = f"Failed to load Universe or determine frame range: {str(e)}"
            logger.error(error_msg)
            raise ValueError(error_msg)
        ...
        if args.summary_only:
            ...
            if run_core and needs_core_run:
                ...
                core_results = analyze_trajectory(run_dir, universe=universe, start_frame=actual_start, end_frame=actual_end)
                if core_results and core_results['status'] == 'success':
                    ...
                    filtering_results = filter_and_save_data(
                        ...
                        start_frame=actual_start, end_frame=actual_end
                    )
            ...
            def run_computation_step(module_flag, run_needed, module_name_comp, run_func, deps_ok=True, dep_err_msg="Dependency failed"):
                nonlocal current_step_success, db_conn, run_dir, universe, actual_start, actual_end
                step_done = False
                if module_flag:
                    if not deps_ok:
                         ...
                         comp_results = run_func(run_dir, universe=universe, start_frame=actual_start, end_frame=actual_end, db_conn=db_conn)
